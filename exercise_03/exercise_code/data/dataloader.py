...
class DataLoader:
    ...
    def __iter__(self):
        ...
        for i, index in enumerate(index_iterator):  # iterate over indices using the iterator
            batch = np.append(batch, self.dataset[index]['data'])
            batch_num = i // self.batch_size

            if len(batch) == self.batch_size:
              batch_dict = {batch_num: batch}
              yield batch_dict  # use yield keyword to define a iterable generator
              batch = np.array([])
            
            if (self.drop_last == False) and (i+1 == len(self.dataset)):
              batch_dict = {batch_num: batch}
              yield batch_dict
    ...
    def __len__(self):
        ...
        if (rest != 0) and (not self.drop_last):
          length += 1
